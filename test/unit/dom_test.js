    ...
    documentViewportProperties = {
      propertiesList : [
        ...
        'document.body.offsetTop', 'document.body.offsetLeft'
			...
			getProperties: function() {
				_ref = this.propertiesList;
				for (_i = 0, _len = _ref.length; _i < _len; _i++) {
					prop = _ref[_i];
					res = [];
					if (!self.screen && prop.match('self.screen')) {
						return;
					}
					if (!document.body && prop.match('document.body')) {
						return;
					}
					res.push(prop);
					if (prop.match('.body') && document.documentElement) {
						res.push(prop.sub('.body', '.documentElement'));
					}
					return res;
				}
			},
      ...
      inspect : function() {
        var props = [];
				...
				_.each(this.getProperties(), function(prop){
					if (eval(prop)) {
						props[prop] = eval(prop);
					}
				});
